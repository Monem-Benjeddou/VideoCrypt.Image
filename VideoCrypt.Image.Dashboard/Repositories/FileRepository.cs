...
namespace VideoCrypt.Image.Dashboard.Repositories
{
    public class FileRepository : IFileRepository
    {
        ...
        public async Task<PaginatedList<string>> ListFilesAsync(int page = 1, int pageSize = 10,string searchQuery="")
        {
            try
            {
                _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer",  GetAccessToken());
                ...
                var response = await _httpClient.GetAsync(new Uri($"{_apiBaseUrl}/api/Image/list?page={page}&pageSize={pageSize}&searchQuery={searchQuery}"));
                if (IsUnAuthorized(response))
                {
                    Logout();
                    throw new UnauthorizedAccessException("Unauthorized user please try logging out then logging int");
