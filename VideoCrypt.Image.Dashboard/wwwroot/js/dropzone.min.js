!function (e, t) {
    ...
    return function () {
        ...
        return function () {
            ...
            var s = {
                ...
                resize: function (e, t, n, r) {
                    ...
                    if (r === "none") {
                        return { srcX: 0, srcY: 0, srcWidth: e.width, srcHeight: e.height, trgWidth: e.width, trgHeight: e.height };
                    }
        ...
        <div class="dz-preview dz-file-preview">
            <div class="dz-image" id="dz-image"><img data-dz-thumbnail/></div>
    ...
    `,                drop: function (e) {
                ...
                addedfile: function (e) {
                    ...
                    if (this.previewsContainer && !this.options.disablePreviews) {
                        e.previewElement = document.createElement('div');
                        e.previewElement.innerHTML = this.options.previewTemplate.trim();
                        e.previewTemplate = e.previewElement;
                        this.previewsContainer.appendChild(e.previewElement);


                        // Update file name and size in preview
                        var nameElements = e.previewElement.querySelectorAll("[data-dz-name]");
                        nameElements.forEach(function (el) {
                            el.textContent = e.upload.filename;
                        });

                        var sizeElements = e.previewElement.querySelectorAll("[data-dz-size]");
                        sizeElements.forEach(function (el) {
                            el.innerHTML = t.filesize(e.size);
                        });

                        // Add remove link if enabled
                        if (this.options.addRemoveLinks) {
                            e._removeLink = document.createElement('a');
                            e._removeLink.className = "dz-remove";
                            e._removeLink.href = "javascript:undefined;";
                            e._removeLink.dataset.dzRemove = true;
                            e._removeLink.textContent = this.options.dictRemoveFile;
                            e.previewElement.appendChild(e._removeLink);

                            // Attach remove file event
                            e._removeLink.addEventListener("click", function (n) {
                                n.preventDefault();
                                n.stopPropagation();
                                if (e.status === Dropzone.UPLOADING) {
                                    if (confirm(t.options.dictCancelUploadConfirmation)) {
                                        t.removeFile(e);
                                    }
                                } else {
                                    if (t.options.dictRemoveFileConfirmation) {
                                        if (confirm(t.options.dictRemoveFileConfirmation)) {
                                            t.removeFile(e);
                                        }
                                    } else {
                                        t.removeFile(e);
                ...
                thumbnail: function (e, t) {
                    ...
                    error: function (e, t) {
                        if (e.previewElement) {
                            ...
                            document.getElementById("errorModalMessage").textContent = t;
                ...
                success: function (e) {
                    ...
                    var dropzoneForm = document.getElementById('my-great-dropzone');
                    var images = document.getElementsByClassName('dz-image');

                    if (dropzoneForm) {
                        dropzoneForm.style.border = 'none';
                        dropzoneForm.style.width = '';
                        dropzoneForm.style.height = '';
                    }

                    // Display image details in modal
                    var imageUrl = e.dataURL; // Assuming e.dataURL contains the image URL
                    var fileName = e.upload.filename;
                    var fileSize = e.size;

                    var modalBody = document.getElementById('errorModalMessage');
                    modalBody.innerHTML = `
        ...
        <p>File Size: ${fileSize}</p>
            <div class="input-group mb-2 mx-2" style="width:97%;">
                ...
                <div class="input-group-append">
                    ...
                    <img src="${imageUrl}" alt="${fileName}" style="max-width: 100%;">
    `;
            ...
            var b = function (e) {
                ...
                return d(i, [{
                    ...
                    key: "init", value: function () {
                        var e = this;
                        "form" === this.element.tagName && this.element.setAttribute("enctype", "multipart/form-data"), this.element.classList.contains("dropzone") && !this.element.querySelector(".dz-message") && this.element.appendChild(i.createElement('<div class="dz-default dz-message"><h1>Upload Your Image Here</h1><button class="dz-button" type="button">'.concat(this.options.dictDefaultMessage, "</button></div>"))), this.clickableElements.length && function t() {
                ...
                , {
                    ...
                    key: "_uploadData", value: function (e, t) {
                        ...
                        // Assign XMLHttpRequest to each file in the batch
                        try {
                        ...
                        // If chunked upload is enabled, assign XMLHttpRequest to the chunk
                        if (e[0].upload.chunked) {
                            e[0].upload.chunks[t[0].chunkIndex].xhr = o;
                        }

                        // Resolve method and URL options
                        var u = this.resolveOption(this.options.method, e);
                        var s = this.resolveOption(this.options.url, e);
                        ...
                        o.ontimeout = function () {
                            r._handleUploadError(e, o, "Request timed out after " + (r.options.timeout / 1000) + " seconds");
                        };
                        ...
                        (o.upload || o).onprogress = function (t) {
